...
export const ActivityCalendarCell: React.FC<IActivityCalendarCell> = ({isActive, dateDisplay = "dd.mm.yyyy", isCurrentMonth, SingleDayActivities, States, givenInfo, borderColor}) =>
{
    ...
    const [prepInfo, setPrepInfo] = useState<IContainedInfo[]>([]);
            ...
            <InfoContainer containedInfoList={prepInfo} appearOn="click" className={`flex size-full `} >
                <div className={`flex content-center justify-around size-full ${isActive? "bg-green-400" : "bg-transparent"} rounded-md flex-wrap opacity-80`}>
                    {prepInfo.filter(x=>(x.type == "EVENT")).map((item)=>(
                        <div key={`${prepInfo.indexOf(item)}`} className="flex size-2 m-[2px] rounded-full self-center border border-border-secondary" style={{backgroundColor: `${item.color}`}}></div>
                    ))}
                </div>
            </InfoContainer>
        </Tooltip>
