...
const InfoContainer = ({
    ...
    return (
        <div 
            className={`relative focus:border-4 ${className!} flex ${tooltipAlignment == 'center' ? "justify-center" : ""}`}  
            onMouseOver={() => {
            ...
            onMouseLeave={() => {
                ...
                if (!mouseOverInfo) {
                    setFocused(false); 
                    setCatchEvents(false);
                }
            }}
        >
            <div 
                className={`size-full ${innerClassName!}`} 
                onClick={() => {
                    setFocused(!focused); 
                    setCatchEvents(!focused);
                }}
            >
                {children}
            </div>
                    ...
                    {containedInfoList.map((item, index) => (
                        <div className="flex flex-col bg-bg-secondary hover:bg-bg-primary px-4 " key={item.id ?? index}>
                            ...
                            <div className="flex justify-between">
                                {item.id != undefined && (
                                    <div 
                                        className="flex text-text-secondary hover:text-text-secondary-accent hover:underline cursor-copy transition-all" 
